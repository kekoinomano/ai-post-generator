	...
	xmlhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if(!consulta.error && consulta.id){
				document.getElementById("gpt3-text").innerHTML='<h2>' + document.getElementsByClassName("gpt3-input")[0].value + '</h2>\n<b' + consulta.choices[0].text;
...
jQuery(document).ready(function($) {
  ...
  // Publish
  $("#response-gpt3-buttons").on("click", "#gpt3-button-create", function() {
  	let type = $(this).attr("data-type");
    text = '<h2>' + document.getElementsByClassName("gpt3-input")[0].value + '</h2>\n<b' + gpt3_text;
		var title = document.getElementById("title").value;
		$.ajax({
	    url : 'admin-ajax.php',
	    type: 'post',
  ...
  // Draft
  $("#response-gpt3-buttons").on("click", "#gpt3-button-draft", function() {
  	let type = $(this).attr("data-type");
    text = '<h2>' + document.getElementsByClassName("gpt3-input")[0].value + '</h2>\n<b' + gpt3_text;
		var title = document.getElementById("title").value;
		$.ajax({
